...
You can set a default resolution for all media parts in a request using the`GenerationConfig`. This is supported by all multimodal models. If a request includes both global and[per-part settings](https://ai.google.dev/gemini-api/docs/media-resolution#per-part-media-resolution), the per-part setting takes precedence for that specific item.  
