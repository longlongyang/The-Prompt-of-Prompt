...
### Go
    ...
    file, err := client.UploadFromPath(ctx, "path/to/sample.mp3", nil)
    if err != nil {
        log.Fatal(err)
    }
    defer client.Delete(ctx, file.Name)
...
### Go
    ...
    gotFile, err := client.Get(ctx, file.Name)
    if err != nil {
        log.Fatal(err)
    }
